...
Describe '00-Copy-Local v2 terminal result contract' {
  ...
  It 'returns one FAIL result and the mapped exit code for an unsafe RepoUrl without invoking git' {
    $scriptPath = Join-Path $PSScriptRoot '../../scripts/00-Copy-Local.ps1'
  ...
  It 'returns WARN without touching the destination or invoking git under WhatIf' {
    $scriptPath = Join-Path $PSScriptRoot '../../scripts/00-Copy-Local.ps1'
  ...
  It 'fails before destination creation when the canonical destination lock is held by another process' -Skip:([Environment]::OSVersion.Platform -eq [System.PlatformID]::Win32NT) {
    ...
    $stagingRoot = Join-Path $tempRoot 'win-mdm-copy-local-staging'
    $escapedScriptPath = $scriptPath.Replace("'", "''")
  ...
  It 'rejects the filesystem volume root as a destination' {
    $scriptPath = Join-Path $PSScriptRoot '../../scripts/00-Copy-Local.ps1'
  ...
  It 'rejects a RepoPath within deployed scripts before creating, deleting, or invoking Git' {
    $scriptPath = Join-Path $PSScriptRoot '../../scripts/00-Copy-Local.ps1'
    $hostPath = (Get-Process -Id $PID).Path
  ...
  It 'does not remove a caller-supplied RepoPath that fails trusted-staging validation' {
    $scriptPath = Join-Path $PSScriptRoot '../../scripts/00-Copy-Local.ps1'
    $hostPath = (Get-Process -Id $PID).Path

...
| `ci` | Step `Secret scan (basic)` failed on Windows PowerShell with empty `$PSScriptRoot` | GitHub Actions PowerShell shell wrapper dot-sourced the script, leaving `$PSScriptRoot` empty in Windows PowerShell 5.1, causing `Join-Path` to receive an empty string | Make root path resolution robust using `$MyInvocation.MyCommand.Path` / `$PSCommandPath` / `Get-Location` fallback | Low (initialization logic only) | `powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\\tools\\secret-scan.ps1` on Windows |

...
| `ci` | Step `Secret scan (basic)` failed on Windows PowerShell with empty `$PSScriptRoot` | GitHub Actions PowerShell shell wrapper dot-sourced the step script, leaving `$PSScriptRoot` empty in Windows PowerShell 5.1 | Make root path resolution robust and pass `-RootPath $env:GITHUB_WORKSPACE` explicitly from CI | Low (initialization logic only) | `powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\\tools\\secret-scan.ps1 -RootPath $env:GITHUB_WORKSPACE` on Windows |
| `ci` | Step `Static checks` failed on Windows PowerShell with empty `$PSScriptRoot` | Same dot-sourcing behavior causes `tools/verify.ps1` to resolve repo root from an empty `$PSScriptRoot` | Make root path resolution robust and pass `-RootPath $env:GITHUB_WORKSPACE` explicitly from CI | Low (initialization logic only) | `powershell.exe -NoProfile -ExecutionPolicy Bypass -File .\\tools\\verify.ps1 -RootPath $env:GITHUB_WORKSPACE` on Windows |
| `ci` | Step `Static checks` failed in Windows PowerShell with `Join-Path` argument error | `Join-Path` with multiple positional segments relies on `-AdditionalChildPath` which does not exist in Windows PowerShell 5.1 | Use `[System.IO.Path]::Combine()` (PS 5.1 compatible) for multi-segment paths | Low (path resolution only) | Re-run CI on Windows PowerShell 5.1 |

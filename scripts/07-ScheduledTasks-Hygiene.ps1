...
function Expand-NormalizePath {
  ...
  foreach ($entry in @(
    ...
    if (-not [string]::IsNullOrWhiteSpace([string]$entry.Value)) {
      ...
      $p2 = [regex]::Replace($p2, $entry.Pattern, $replacement, [System.Text.RegularExpressions.RegexOptions]::IgnoreCase, (New-TimeSpan -Milliseconds 100))
    }
  }

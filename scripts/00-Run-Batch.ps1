...
function New-BatchProfileWorkspace {
  ...
  if ($isElevatedWindows) {
    ...
    $directory = Join-Path $programData ("WinMdmSecurityHardeningKit-Batch-{0}" -f [guid]::NewGuid().ToString('N'))
  ...
  $directory = Join-Path ([System.IO.Path]::GetTempPath()) ("win-mdm-batch-{0}" -f [guid]::NewGuid().ToString('N'))
  [void][System.IO.Directory]::CreateDirectory($directory)
  return $directory
}

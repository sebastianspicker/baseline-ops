...
function Assert-CopyLocalBootstrapPathTrust {
  ...
  $writeMask = [Security.AccessControl.FileSystemRights]::Write -bor [Security.AccessControl.FileSystemRights]::Modify -bor
...
Initialize-V2Context -ScriptName '00-Copy-Local.ps1' -BoundParameters $PSBoundParameters
...
function Get-CopyLocalStagingRoot {
  ...
  if ([Environment]::OSVersion.Platform -eq [System.PlatformID]::Win32NT) {
    ...
    return (Join-Path $commonApplicationData 'WinMdmSecurityHardeningKit\CopyLocalStaging')
  }
  ...
  return (Join-Path $temporaryRoot 'win-mdm-copy-local-staging')
}

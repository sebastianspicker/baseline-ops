...
Default: %ProgramData%\WinMdmSecurityHardeningKit\SupportBundles
...
Default: %ProgramData%\WinMdmSecurityHardeningKit\SupportBundles\_extracted
...
.EXAMPLE
PS> $r = .\10-SupportBundle-Parser.ps1 -SupportDir 'C:\PATH\TO\SUPPORT' -ExtractRoot 'C:\PATH\TO\SUPPORT\_extracted' -PassThru
...
$commonApplicationData = if ($PSVersionTable.PSEdition -eq 'Core' -and -not $IsWindows) {
  ...
  # ProgramData value.  Production Windows resolves CommonApplicationData.
  $env:ProgramData
} else {
  [Environment]::GetFolderPath([Environment+SpecialFolder]::CommonApplicationData)
}
if ([string]::IsNullOrWhiteSpace($commonApplicationData)) { throw 'CommonApplicationData could not be resolved.' }
$defaultSupportDir = Join-Path (Join-Path $commonApplicationData 'WinMdmSecurityHardeningKit') 'SupportBundles'
...
if ([string]::IsNullOrWhiteSpace($ExtractRoot)) { $ExtractRoot = Join-Path $SupportDir '_extracted' }
...
foreach ($producerError in @($summaryErrors)) {
...
$legacyFindings = Invoke-FindingsCheck -ZipMarkerPresent $zipMarkerPresent -Proofs $proofStatus -WorkDirExists $workDirExists -KbStatus $kbInfo

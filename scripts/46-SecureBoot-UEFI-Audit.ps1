...
# 2. Determine firmware type from registry / environment
try {
  $fwEnv = Get-CimInstance -ClassName Win32_OperatingSystem -ErrorAction Stop
  if ($fwEnv) {
    # BootDevice starting with \Device\HarddiskVolume typically indicates UEFI
    # FirmwareType in newer builds: 1 = BIOS, 2 = UEFI
    $firmwareType = 'UEFI'
    try {
      $fwReg = Get-ItemProperty -LiteralPath 'HKLM:\SYSTEM\CurrentControlSet\Control\SecureBoot' -ErrorAction Stop
      if ($null -eq $fwReg) {
        $firmwareType = 'Legacy BIOS'
      }
    } catch {
      # SecureBoot key missing likely means legacy BIOS
      $firmwareType = 'Legacy BIOS'
    }
  }
} catch {

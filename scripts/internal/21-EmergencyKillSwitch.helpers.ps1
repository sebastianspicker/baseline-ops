# Helper functions extracted from 21-EmergencyKillSwitch.ps1.
# Dot-source after the entry script creates $Run and imports shared modules.
...
function Get-KillSwitchLockDirectory {
  ...
  $directory = Join-Path $trustedParent 'WinMdmSecurityHardeningKit-EmergencyKillSwitch'
...
function Enter-KillSwitchRemediationLock {
  ...
  if ([System.Environment]::OSVersion.Platform -ne [System.PlatformID]::Win32NT) {
    if ([string]::IsNullOrWhiteSpace($LockDirectory)) {
      $LockDirectory = Join-Path ([System.IO.Path]::GetTempPath()) 'WinMdmSecurityHardeningKit-EmergencyKillSwitch'
    }

# Helper functions extracted from 11-IOC-Sweep-Defender.ps1.
# Dot-source only after the script imports its lib modules.
...
function Find-MpCmdRun { $programFiles = [Environment]::GetFolderPath([Environment+SpecialFolder]::ProgramFiles); if ([Environment]::OSVersion.Platform -ne [PlatformID]::Win32NT) { $programFiles = $env:WINMDM_TEST_PROGRAM_FILES }; if ([string]::IsNullOrWhiteSpace($programFiles)) { return $null }; foreach ($relative in @('Windows Defender\MpCmdRun.exe','Microsoft Defender\MpCmdRun.exe')) { $candidate = Join-Path $programFiles $relative; if (Test-Path -LiteralPath $candidate -PathType Leaf) { return $candidate } }; return $null }
function New-IocRegex { param([Parameter(Mandatory)][string]$Pattern,[Parameter(Mandatory)][string]$Label); if ($Pattern.Length -gt 1024) { throw "IOC $Label regex exceeds the 1024-character limit." }; try { return New-Object System.Text.RegularExpressions.Regex($Pattern, [System.Text.RegularExpressions.RegexOptions]::CultureInvariant, (New-TimeSpan -Milliseconds 250)) } catch { throw "IOC $Label regex is invalid: $($_.Exception.Message)" } }
...
function Add-IocSourceStatus {
  ...
  if ($Attempted -and -not $Succeeded -and -not [string]::IsNullOrWhiteSpace($ErrorMessage)) { $msg = "{0} source failed: {1}" -f $Name, $ErrorMessage; $Proof.Errors += $msg; [void](Add-Finding -Code 'IOC-SourceFailed' -Severity 'High' -Message $msg -Extra @{ Source = $Name }) }
}
